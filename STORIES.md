# Implementation Stories — xllm

> Project: **xllm** — a from-scratch LLaMA inference engine in C++ on Apple MLX (the C++ core library, not mlx-lm), served behind an OpenAI-compatible HTTP API with continuous batching.
...
The spec defines 11 phases (0–10). Stories preserve that dependency ordering but are grouped into six sprints. Each story carries a stable ID (`XLLM-NNN`), maps to one or more modules from the spec's decomposition table, and — where the spec defines a per-phase golden-reference assertion — embeds that exact check as a required acceptance criterion.

The golden-reference discipline is threaded throughout: **XLLM-002** stands up the Python reference dump early, and every numerically-relevant story afterward asserts tensor-closeness (fp16 rel ~1e-2) or exact greedy-token equality against it.
...
- **Highest-risk stories**: XLLM-010 (single→batch KV cache), XLLM-016/017/018 (continuous-batching scheduler), XLLM-006 (RoPE conventions), XLLM-013 (eval placement), XLLM-019 (batch-shape bucketing/recompilation), XLLM-021 (tokenizer streaming).
...
- **Framework**: unit tests use a single C++ framework (recommended **doctest** or **Catch2** — header-only, trivial CMake integration), registered with **`ctest`**. XLLM-001 wires the framework into the build; XLLM-005 adds the `.npy` reference-compare helper on top.
...
### XLLM-001: CMake project links MLX and runs "hello array"

- **Size**: M
- **Dependencies**: none
- **Files/Modules**: `CMakeLists.txt`, `apps/xllm_cli.cpp` (stub), top-level build scaffolding
...
### XLLM-003: `ModelConfig` parsed from `config.json`

- **Size**: S
- **Dependencies**: XLLM-001
...
### XLLM-004: Load, sanitize & fp16-cast weights from safetensors

- **Size**: L
- **Dependencies**: XLLM-001, XLLM-003
...
### XLLM-005: C++ tensor-compare test harness

- **Size**: S
- **Dependencies**: XLLM-002
...
### XLLM-006: Embedding + RMSNorm + Q/K/V projections + RoPE (HIGH RISK)

- **Size**: L
- **Dependencies**: XLLM-004, XLLM-005
...
### XLLM-007: Single decoder block — attention + SwiGLU MLP + residuals

- **Size**: L
- **Dependencies**: XLLM-006
...
### XLLM-008: Full forward pass to logits (MILESTONE: numerically correct)

- **Size**: M
- **Dependencies**: XLLM-007
...
### XLLM-009: Single-sequence `KVCache` (prefill + decode append)

- **Size**: M
- **Dependencies**: XLLM-008
...
### XLLM-012: KV admission gate (OOM guard)

- **Size**: S
- **Dependencies**: XLLM-011
...
### XLLM-013: Batched decode equivalence test (eval-placement guard) (HIGH RISK)
...
### XLLM-014: Sampling ops — greedy, temperature, top-k, top-p (as graph ops)
...
### XLLM-015: `xllm-cli` generation loop (MILESTONE: it generates text)

- **Size**: M
- **Dependencies**: XLLM-013, XLLM-014
- **Files/Modules**: `apps/xllm_cli.cpp`
- **Description**: Wire the single-stream generation loop in the CLI: sample → append → (placeholder) detokenize → stream to stdout → until EOS or `max_tokens`. Until the C++ tokenizer lands (XLLM-021), feed pre-tokenized IDs and dump token IDs / use a Python decode step.
- **Acceptance Criteria**:
  ...
  - [ ] `xllm-cli` streams tokens incrementally to stdout (IDs acceptable until tokenizer lands).
...
### XLLM-016: Single GPU worker thread + three-queue skeleton (HIGH RISK)
...
### XLLM-017: Prefill pass — left-pad, chunked prefill, merge into decode cache (HIGH RISK)

- **Size**: L
- **Dependencies**: XLLM-016, XLLM-012
...
### XLLM-018: Steady-state decode step — single async_eval + 1-step lookahead (HIGH RISK)

- **Size**: XL
- **Dependencies**: XLLM-017
...
### XLLM-019: Ragged-batch additive fp16 mask + batch-size bucketing (HIGH RISK)

- **Size**: L
- **Dependencies**: XLLM-018
...
### XLLM-020: Scheduler correctness & throughput validation (MILESTONE: concurrent batching works)

- **Size**: M
- **Dependencies**: XLLM-019
...
### XLLM-021: C++ tokenizer — encode/decode + chat template (HIGH RISK: streaming)

- **Size**: L
- **Dependencies**: XLLM-002 (for round-trip fixtures)
...
- **Acceptance Criteria**:
  ...
  - [ ] `xllm-cli` (XLLM-015) can now stream real text instead of token IDs.

### XLLM-022: cpp-httplib OpenAI server — routes, JSON parse, enqueue

- **Size**: L
- **Dependencies**: XLLM-020, XLLM-021
- **Files/Modules**: `server/http_server.{h,cpp}` (cpp-httplib, nlohmann/json), `apps/xllm.cpp`
...
### XLLM-023: SSE streaming + cancellation (MILESTONE: OpenAI server live)

- **Size**: L
- **Dependencies**: XLLM-022
...
### XLLM-024: Hardening & ops — config, metrics, graceful shutdown, error shapes

- **Size**: L
- **Dependencies**: XLLM-023
- **Files/Modules**: `server/http_server.{h,cpp}`, `runtime/worker.{h,cpp}`, `apps/xllm.cpp`, `scheduler/scheduler.{h,cpp}`
...
### XLLM-025: Optional 4-bit quantization via `quantized_matmul`

- **Size**: M
- **Dependencies**: XLLM-008 (numerically-correct fp16 baseline); slots in any time after, independent of server work
...
```
XLLM-001 ─┬─> XLLM-003 ─> XLLM-004 ─┬─> XLLM-006 ─> XLLM-007 ─> XLLM-008 ─┬─> XLLM-009 ─> XLLM-010 ─> XLLM-011 ─┬─> XLLM-012
XLLM-002 ─┴─> XLLM-005 ──────────────┘                                     │                                   ├─> XLLM-013
                                                                           │                                   └─> (XLLM-025 optional, any time after 008)
                                       XLLM-008 ─> XLLM-014 ───────────────┘
XLLM-013 + XLLM-014 ─> XLLM-015  (CLI generates text)
XLLM-013 + XLLM-014 ─> XLLM-016 ─> XLLM-017 (+XLLM-012) ─> XLLM-018 ─> XLLM-019 ─> XLLM-020  (concurrent batching)
XLLM-002 ─> XLLM-021
XLLM-020 + XLLM-021 ─> XLLM-022 ─> XLLM-023  (OpenAI server live)
XLLM-023 ─> XLLM-024
```

**Critical path**: 001/002 → 003 → 004 → 006 → 007 → 008 → 009 → 010 → 011 → (012) → 013 → 016 → 017 → 018 → 019 → 020 → 022 → 023 → 024.

**Key milestones (from spec)**: XLLM-008 (numerically-correct model), XLLM-015 (it generates text), XLLM-020 (concurrent batching works), XLLM-023 (OpenAI server live).

**Parallelizable**: XLLM-002 / XLLM-005 (reference harness) run alongside XLLM-001/003/004. XLLM-014 (sampler) can start as soon as XLLM-008 lands. XLLM-021 (tokenizer) can be built any time after XLLM-002 and is only *required* by XLLM-022. XLLM-025 (quantization) is independent of all server work.
...
|---|---|---|---|
| 1 | **Eval placement / serialization (highest)** | XLLM-013, XLLM-018 | Exactly **one `async_eval` per decode step** over the whole batch; treat "evals per step" as a guarded, instrumented invariant; never per-row/per-layer. |
| 2 | **Regraph/recompile on changing batch shape** | XLLM-019 | Bucket `B` to {1,2,4,8,16,32} with masked dummy rows; keep `S_cap` on the 256 step; validate uncompiled before adopting `mx::compile`. |
| 3 | **RoPE per-row offset overload missing** | XLLM-006 | Assert the `fast::rope(const array& offset, …)` overload exists on the pinned MLX; fallback is `freqs` materialization. |
| 4 | **Silent numerical bugs (RoPE base/interleave, weight transpose, GQA repeat)** | XLLM-006, XLLM-007, XLLM-008 | Golden-reference comparison at every phase (XLLM-002 + XLLM-005), exact argmax/token checks. |
| 5 | **Graph/memory growth during long prefill** | XLLM-017 | `eval(cache.state)` at each prefill chunk boundary; monitor `mx::metal::get_active_memory()`; `clear_cache()` if high-watermark creeps. |
| 6 | **OOM (Metal allocates until failure)** | XLLM-012, XLLM-024 | KV admission gate (32 KiB/token projection vs budget) + bounded `waiting` queue (429 on overflow). |
| 7 | **Tokenizer streaming emits broken UTF-8** | XLLM-021, XLLM-023 | Incremental UTF-8 / byte-BPE decode so SSE never emits partial characters. |
| 8 | **MLX thread-safety** | XLLM-016 | All MLX work on the single worker thread; HTTP/test threads touch only their own `Request`. |
| 9 | **Boolean mask >2³¹ bug (#2894)** | XLLM-019 | Use additive fp16 masks, never boolean. |

---

## Recommended review points

- **After XLLM-008**: Confirm the model is numerically correct (argmax matches mlx-lm) before building any caching/serving on top of it. Stop-the-line if reference checks fail.
- **After XLLM-013**: Confirm batched cache equivalence and the single-eval invariant before committing to the scheduler.
- **After XLLM-020**: Confirm continuous batching preserves outputs and scales throughput before wiring the HTTP layer.
- **After XLLM-023**: Confirm OpenAI client parity (streaming + non-streaming + cancellation) before hardening.
