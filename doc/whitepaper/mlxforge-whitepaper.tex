...
Swift, and Rust as a shared library --- the library, not the bundled HTTP server
or CLI, is the product.

The defining engineering constraint is that the failure mode of an inference
engine is \emph{silent numerical garbage, not a crash}. Every numerically
...
weights, with greedy decoding required to match token-for-token. This paper
documents the architecture and the mathematics in enough detail to reproduce or
...
\begin{itemize}[nosep]
  \item \emph{Array-framework bindings} (\code{mlx-c}, \code{mlx-rs}) are
        ...
        --- scheduler, KV cache, tokenizer, sampling --- yourself.
...
\emph{wrong} text. The bug is invisible to inspection. The whole engineering
discipline of \code{mlxforge} follows from this: anything touching the forward
pass, the KV cache, or sampling must be validated against a golden reference, not
eyeballed. Chapter~\ref{ch:testing} describes the methodology; the rest of the
paper repeatedly notes where a numerical invariant is load-bearing.
...
build on top of it: \code{libmlxforge} (the C-ABI shared library --- the product),
the \code{mlxforge} server, and the \code{mlxforge-cli} tool. The server and CLI
...
\code{mlxforge::resolve\_model\_dir} (\code{src/core/model\_source}). Resolution is
...
compare-and-swap on model state. Concurrency correctness is achieved
\emph{structurally}, not by locking the GPU.
...
boundaries; a disconnected client causes immediate eviction.
...
\section{Goals}
The scheduler must serve concurrent users on one GPU worker, admit new work into an
active batch and evict finished work dynamically, keep one GPU graph alive rather
than recompiling per request, and --- the highest-severity invariant --- issue
exactly one \code{async\_eval} per decode step covering the whole batch. The design
follows the continuous-batching lineage of Orca~\citep{yu2022orca} and
...
\code{Worker::decode\_steps()} counts these evaluations; under load the step count
is far below the total token count, which is the operational proof that batching is
actually happening.
...
(\code{src/cache/kv\_budget}) is refused; combined with the bounded waiting queue
(which returns \code{429} on overflow), this prevents out-of-memory conditions
under load.
...
$\texttt{rope\_freq\_factors}$, so the engine simply multiplies:
...
count --- with no per-row 3D positions needed in the cache.
...
deeper into the language tower. DeepStack is a prefill-only phenomenon; a generated
...
height, width). Crucially, for a text token $t=h=w$, so every axis gives the same
...
\code{dump\_ref.py} to emit the intermediate tensor and assert against it --- which
is how the front-half embedding/post-norm/RoPE'd-$Q/K$ bugs were originally found.
...
\code{mlxforge} demonstrates that a fully batched LLM engine can be embedded
in-process across languages on Apple Silicon, without sacrificing numerical
fidelity. The architecture is organized around one structural fact --- MLX arrays
are thread-bound, so one worker owns the GPU --- and one discipline --- silent
numerical error is the enemy, so every sensitive stage is golden-gated.

Open directions include: paged attention (when an MLX primitive exists) to remove
the left-padding waste and enable prefix sharing; per-row 3D positions in the
batched cache to batch vision \emph{prefill}, not just decode; and additional model
families behind the same \code{DecoderModel} hooks.
